...
/* restore active tab from ?tab= (used after attendance/overtime filter or pagination reloads) */
(function() {
    const tab = new URLSearchParams(window.location.search).get('tab');
    if (!tab) return;
    const tabBtn = document.querySelector(`.emp-tab[data-tab="${tab}"]`);
...
function closeLightbox()      { closeModal('lightboxOverlay', 'lightboxPanel'); }
function closeNote()          { closeModal('noteOverlay', 'noteModal'); }
function closeOvertimeModal() { closeModal('otOverlay', 'otPanel'); }
...
function deleteOvertime(id, label) {
    pendingDelete = { url: getOtDeleteUrl(id) };
    document.getElementById('delName').textContent = label;
    openModal('delOverlay', 'delModal');
}
...
function approveRecord(id) {
    if (!confirm('اعتماد هذا السجل سيحذف صورته نهائياً من النظام. هل تريد المتابعة؟')) return;
    const fd = new FormData();
    fd.append('csrfmiddlewaretoken', CSRF);
    fetch(getAttApproveUrl(id), { method: 'POST', body: fd })
        .then(r => r.json())
        .then(d => { if (d.ok) window.location.reload(); });
}
...
if (btnApproveAll) {
    btnApproveAll.addEventListener('click', function() {
        if (!confirm('سيتم اعتماد جميع السجلات المفلترة غير المعتمدة وحذف صورها نهائياً. هل تريد المتابعة؟')) return;
        const fd = new FormData();
        fd.append('csrfmiddlewaretoken', CSRF);
        fd.append('employee',  ATT_FILTERS.employee);
        fd.append('date_from', ATT_FILTERS.date_from);
        fd.append('date_to',   ATT_FILTERS.date_to);
        fetch(URL_ATT_APPROVE_BULK, { method: 'POST', body: fd })
            .then(r => r.json())
            .then(d => { if (d.ok) { alert(`تم اعتماد ${d.count} سجل`); window.location.reload(); } });
    });
}

function openLightbox(url) {
    document.getElementById('lightboxImg').src = url;
    openModal('lightboxOverlay', 'lightboxPanel');
}

function openNote(el) {
    document.getElementById('noteText').textContent = el.dataset.note;
    openModal('noteOverlay', 'noteModal');
}

/* ── Overtime ── */
function openAddOvertime() {
    document.getElementById('otForm').reset();
    openModal('otOverlay', 'otPanel');
}

function submitOvertime() {
    const fd = new FormData(document.getElementById('otForm'));
    fetch(URL_OT_CREATE, { method: 'POST', body: fd })
        .then(r => r.json())
        .then(d => { if (d.ok) window.location.reload(); else alert(d.error || 'حدث خطأ'); });
}
...
document.addEventListener('keydown', e => {
    if (e.key === 'Escape') {
        ...
        closeLightbox(); closeNote(); closeOvertimeModal();
    }
});
